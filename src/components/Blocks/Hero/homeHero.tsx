      ...
      <Flex align={"center"} pos={"absolute"} top={0} left={0} bg={"#021821d6"} w={'100%'} h="100%" zIndex={3}>
        <Grid templateColumns={{base: 'repeat(1, 1fr)', md: 'repeat(3, 1fr)', lg: 'repeat(7, 1fr)'}} gap={12}  px={{ base: 8, '2xl': 36 }}>
          <GridItem colSpan={{base: 2, md: 3,  lg: 5, xl: 3}}>
            <Text fontSize={{base: 52, md: 64, lg: 80}} textAlign={{base:"left",  xl: "center"}} fontWeight={600} color={"white"} lineHeight={1.1}> {data[0]?.data?.title[0]?.text}</Text>
            <Text color={"whiteAlpha.700"} textAlign={{base:"left",  xl: "center"}} fontSize={{base: 12, sm: 14, md: 16}} fontFamily={"Montserrat"}> {data[0]?.data?.description}</Text>
        ...
        <Box justifyContent={"flex-end"} display={{base: 'none', lg: 'flex'}} pos={"absolute"} bottom={{md: '-500px', xl: "-300px"}} right={{md: 0, lg: "-200px", xl: 24}}>
          <Image w={{md: "80%", lg: "100%", xl: "130%"}} src={data[0]?.data?.cutout_image?.url} alt="Rev George"/>
        </Box>
      </Flex>
