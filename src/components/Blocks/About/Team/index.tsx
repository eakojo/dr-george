...
const Team: FC = () => {
  return (
    <Box
      ...
      pb={24}
    >
        <Box  color="base.black" fontSize={52} lineHeight={1.2}>
            <Text>Our Team</Text>
        </Box>
        <Box mt={12}>
            <Grid templateColumns={"repeat(12, 1fr)"} gap={16} my={6}>
                {teamPeople.map(item =>                 <Box key={item.name} as={GridItem} colSpan={4}>
                    <Box bg="gray.200" w="full" h={96}></Box>
                    <Text fontWeight={600} color={"gray.600"} fontSize={32}>{item.name}</Text>
                    <Text fontSize={28} color={"gray.500"}>{item.role}</Text>
                </Box>)}
